...
def treinar_e_avaliar(X_train, y_train, X_test, y_test):
    print("=" * 57)
    print(f"TREINAMENTO KNN  (K={K_NEIGHBORS}, métrica={METRIC})")
    print("=" * 57 + "\n")

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    modelo = KNeighborsClassifier(n_neighbors=K_NEIGHBORS, metric=METRIC, weights='uniform')
    ...
    with open(f"{OUTPUT_DIR}/knn_relatorio_classificacao.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"K={K_NEIGHBORS}  |  métrica={METRIC}\n\n")
